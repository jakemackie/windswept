	...
	try {
		...
		if (userEconomy?.lastDailyRewardClaimed) {
			...
			if (now.getTime() - lastClaimed.getTime() < 24 * 60 * 60 * 1000) {
				interaction.reply(
					`You've already claimed your daily reward. You can claim again <t:${Math.floor((lastClaimed.getTime() + 24 * 60 * 60 * 1000) / 1000)}:R>.`,
				);
				return;
			}
		}
		...
		await interaction.reply(
			`Claimed your daily reward of **${amountToReward}** coins!`,
		);
	} catch (error) {
		console.error(error);
